...
## UPDATE — 2026-06-26 23:15 AEST

**cve-submissions** — commit `583b6a2` (Loop 21, 23:06): GSWA-015 stub, named_pipe_enum fix, 8th MuseHub precedent.

### GSWA-2026-015 — NVIDIA Stub Created, SDDL Authoritative

**05_NVIDIA_STUB.md** committed to FINAL_SUBMISSIONS. Definitive SDDL confirmed:
...
- `D:NO_ACCESS_CONTROL` = **NULL DACL** (explicit kernel notation, not inferred)

This is the exact SDDL returned by `ConvertSecurityDescriptorToStringSecurityDescriptor()` on `NvWksServiceDdisplayPipe`. Matches our RADON live probe from this session (empty SDDL string → NULL DACL). Submission stub at CVSS 5.5 structural / 7.8 if privileged opcode confirmed. **Vendor: psirt@nvidia.com.** Status: STUB pending protocol analysis.

### named_pipe_enum.ps1 NULL DACL Fix (Loop 21)

gwu07 loop independently found the same false-negative we identified this session. Tool now has `Is-NullDacl()` function + **CRITICAL priority tier** for NULL DACL pipes. NvWksServiceDdisplayPipe previously classified LOW; now CRITICAL. Our RADON autonomous probe (P/Invoke GetSecurityInfo → empty SDDL) was the correct detection method — now incorporated into the tool.

**Updated named_pipe_enum.ps1** in origin/master. Extracting to local working tree for future RADON scans.

### MuseHub 8th Precedent — CVE-2026-2998 (eAI ERP F2 CWE-426)

eAI ERP F2 — CWE-426 (Untrusted Search Path) assigned 2026. Added to MuseHub prior art table. **8 precedents now.** MuseHub probability holds at 80-85%.

### AlwaysInstallElevated — Permanently Blocked

Third dead-end at N%8=5 (loops 5, 13, 21). Both HKLM+HKCU NOT SET on LAPTOP-R32M8MLI. Moved to blocked list permanently. N%8=5 slot will be reassigned before loop 29.

### LOOP_STATE — loop_n=21, consecutive_dry=0

Hot vectors: Wondershare (95%), MuseHub (80-85%), HKClipSvc, Everything Service, GSWA-015 NVIDIA, AsusCertService. GSWA-015 now formally in hot_vectors with SDDL and vendor. Pending verifications unchanged: Wondershare email STILL NOT SENT (highest priority).

### Submission Queue — George's Immediate Actions

1. **security@wondershare.com** — HIGHEST PRIORITY. 95%. Run: `python techniques/vendor_email_gen.py --vendor wondershare`
2. **security@flexerasoftware.com** — FlexNet DoS CVSS 5.5 CWE-400. 3-line PoC. Submit now.
3. **security@musescore.com** — MuseHub 80-85%, 8 precedents. After Wondershare.
4. **psirt@nvidia.com** — GSWA-015 stub ready in 05_NVIDIA_STUB.md. Hold until protocol analysis (CVSS 5.5 now, 7.8 target).

New cve-submissions HEAD: `583b6a2`

---

## UPDATE — 2026-06-26 23:22 AEST

**RADON autonomous scan** — everything_ipc_probe.ps1 executed on RADON_Laptop1.

### GSWA-2026-014 Everything Service — Write Primitive Claim Unconfirmed

**Probe results (everything_ipc_probe.ps1, RADON):**
```
[1]  connect only           CONN_OK_NO_RESP
[2]  version (01 00 00 00)  CONN_OK_NO_RESP
[3]  IS_DB_LOADED (01 04)   CONN_OK_NO_RESP
[4]  IS_DB_BUSY (02 04)     CONN_OK_NO_RESP
[5]  QUERY2 *.exe           ERR: Pipe broken (server parses QUERY2 magic, rejects bad payload)
[6]  QUERY2 password        ERR: Pipe broken (same)
[7]  QUERY old (4c44504e)   CONN_OK_NO_RESP
[8]  null 00 00 00 00       CONN_OK_NO_RESP
[9]  REBUILD_DB 0x500       CONN_OK_NO_RESP
[10] SAVE_DB 0x501          CONN_OK_NO_RESP
```

**Everything-1.5a.db mtime after probe: UNCHANGED** (still 04:00:52, 62,426,933 bytes).

**Conclusions:**
- **CONN_OK_NO_RESP** = server accepted connection + message, sent nothing back, closed. Not a parse/execute confirmation.
- **0x500 REBUILD_DB and 0x501 SAVE_DB**: no observable filesystem effect on RADON. Write primitive claim from Loop 15 is **unconfirmed** on this install. Possible the write ops are WM_COPYDATA-only (not pipe-based) or require a well-structured payload header before the opcode.
- **QUERY2 pipe-break confirms parsing**: server reads the COPYDATAQUERY2 magic (0x4c44504f) and parses the body — broken pipe = malformed payload rejected by parser. With a correctly structured QUERY2 payload, standard user could enumerate the Everything index (runs as LocalSystem, may index files the caller can't access directly) → **CWE-200 information disclosure via filesystem index enumeration.**
- **GSWA-014 assessment revised**: CWE-200 (info disclosure via QUERY2) confirmed as viable. Write primitive (CWE-732) unconfirmed — needs correct QUERY2 framing for 0x500/0x501 or WM_COPYDATA test.

### Everything Service Install on RADON

- Binary: `C:\Program Files (x86)\Everything 1.5a\Everything.exe -svc`
- DB: `C:\Users\Ghaleb Jomma\AppData\Local\Everything\Everything-1.5a.db` (62 MB, LocalSystem writes it)
- Service runs as LocalSystem but db is in user's AppData — on RADON this weakens the privilege gap (Ghaleb Jomma owns the db anyway)
- **On gwu07**: if Everything indexes all drives as LocalSystem and the db is in a different user's profile, QUERY2 read → CWE-200 cross-user index exposure is stronger
